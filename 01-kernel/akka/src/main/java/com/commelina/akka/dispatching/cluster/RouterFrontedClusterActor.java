package com.commelina.akka.dispatching.cluster;
...
import com.commelina.akka.dispatching.Rewrite;
...
public class RouterFrontedClusterActor extends AbstractActor implements Rewrite {
    ...
    public final Receive createReceive() {
        return receiveBuilder()
                // 客户端请求
                .match(com.commelina.akka.dispatching.proto.ApiRequest.class, r -> {
                ...
                .match(com.commelina.akka.dispatching.proto.ActorNotify.class, n -> MessageAdapter.sendNotify(myRouterId, NotifyMessage.newMessage(
                        n.getOpcode(),
                        n.getUserId(),
                        () -> n.getMessage().toByteArray()
                )))
                .match(com.commelina.akka.dispatching.proto.ActorBroadcast.class, b -> MessageAdapter.sendBroadcast(myRouterId, BroadcastMessage.newBroadcast(
                        b.getOpcode(), b.getUserIdsList(), () -> b.getMessage().toByteArray()
                )))
                .match(com.commelina.akka.dispatching.proto.ActorWorld.class, w -> MessageAdapter.sendWorld(myRouterId, WorldMessage.newMessage(
                        w.getOpcode(), () -> w.getMessage().toByteArray())))
                // 重定向请求
                .match(com.commelina.akka.dispatching.proto.ApiRequestForward.class, rf -> {
                    ...
                    if (targetSystem != null) {
                        ...
                        if (target != null) {
                            ...
                            // ask with pipe
                            CompletableFuture<com.commelina.akka.dispatching.proto.ActorResponse> transformed = CompletableFuture
                                    .allOf(askFuture)
                                    .thenApply(v -> (com.commelina.akka.dispatching.proto.ActorResponse) askFuture.join());
...
//                    clusterNodeRouters.inverse().remove(t.getActor());
    ...
    @Override
    public int selectActorSeed(com.commelina.akka.dispatching.proto.ApiRequest ask) {
        return clusterNodeRouters.keySet().iterator().next();
    }

    @Override
    public int selectActorSeed(com.commelina.akka.dispatching.proto.ApiRequestForward forward) {
        return clusterNodeRouters.keySet().iterator().next();
    }
